...
def main(filename: str):
    ...
    for c_id, course in enumerate(df_courses.to_dict(orient="records")):
        ...
        course_info_title = [
            el.text for el in soup.find_all(class_="about_training_box_title")
        ]
        course_info_desc = [
            ". ".join([text_val.text for text_val in el.ul.contents])
            for el in soup.find_all(class_="about_training_box_description")
        ]
        course_info = dict(zip(course_info_title, course_info_desc))
        course_dict = {
            "prerequisites": course_info["Wymagania"]
            if "Wymagania" in course_info
            else "",
            "addressedTo": course_info["Dla kogo?"]
            if "Dla kogo?" in course_info
            else "",
        }
...
if __name__ == "__main__":
    ...
    parser.add_argument(
        "-f", "--filename", help="Path to the filename with courses."
    )
