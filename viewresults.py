...
def read_pandas(dir_path):
    ...
    paths = list(
        pathlib.Path(dir_path).glob("*/CRFEntityExtractor_report.json")
    )
...
intent_df, entity_df = read_pandas("results_output_md")
...
selected_config = st.sidebar.multiselect(
    "Wybierze konfiguracje do porównania",
    possible_configs,
    default=possible_configs,
)
show_raw_data = st.sidebar.checkbox("Pokaż statystyki")

subset_df = intent_df.loc[lambda d: d["config"].isin(selected_config)].melt(
    "config"
)
...
subset_df = entity_df.loc[lambda d: d["config"].isin(selected_config)].melt(
    "config"
)
